import { bookInventoryFields } from '@/constants/formFields/bookFields'
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from '@/components/ui/accordion'
import { Button } from '@/components/ui/button'
import { Trash2, Plus, Save, Copy } from 'lucide-react'
import { Controller, FormProvider, useFieldArray, useForm } from 'react-hook-form'
import { BookInventoryType, bookInventoryValidators } from '@/validators/inventoryValidator'
import { bookInventoryFieldsDefaults } from '@/constants/data'
...
export interface BookInventoryForm {
  inventory: BookInventoryType[]
}

const bookInventoryResolver = z.object({
  inventory: z.array(bookInventoryValidators),
})

const BookInventory = () => {
  const { updateProductInventory, addNewProductInventory } = useAddProduct();
  const methods = useForm<BookInventoryForm>({
    resolver: zodResolver(bookInventoryResolver),
    defaultValues: {
      inventory: [bookInventoryFieldsDefaults],
    },
  });

  const { fields, remove, append } = useFieldArray<BookInventoryForm, 'inventory'>({
  ...
  const handleCopy = (index: number) => {
    ...
    append(rest);
    addNewProductInventory('book', index);
  }

  useEffect(() => {
    const setProductValues = () => {
      const product = useAddProduct.getState().products.book
      const inventory = product.inventory as BookInventoryType[]
  ...
  const handleSubmit = (data: BookInventoryForm) => {
    updateProductInventory('book', data.inventory)
          ...
          <div className="border-b border-outline-variant bg-surface-container-low px-5 py-4 sm:px-8">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button className="self-end bg-secondary font-bold text-on-secondary hover:bg-secondary/90"
                onClick={() => append(bookInventoryFieldsDefaults)} type="button">
                <Plus />
                Add new variant
              </Button>
            </div>
              ...
              <AccordionItem
                ...
                <div className="relative">
                  ...
                  <div className="absolute right-8 top-1/2 z-10 flex -translate-y-1/2 shrink-0 items-center gap-1">
                    ...
                    <Button className="self-end font-bold " onClick={() => handleCopy(index)} type="button">
                      <Copy />
                      Copy
                    </Button>
                  </div>
                ...
                <AccordionContent className="space-y-5 mt-3">
                  <div className="space-y-5 rounded-lg border border-outline-variant bg-surface-container-low p-4 sm:p-5">
                    ...
                    <div className="grid grid-cols-1 gap-7 md:grid-cols-2">
                      {bookInventoryFields.map((item, idx) => {
